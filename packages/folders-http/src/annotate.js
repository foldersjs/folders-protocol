...
var Annotation = function () {
  ...
  var addNote = function (path, note, view, cb) {
    ...
    db.run(
      "INSERT OR REPLACE INTO notes (ID, path, note, view) VALUES ((SELECT ID FROM notes WHERE path = ?), ?, ?, ?)",
      path,
      path,
      note,
      view,
  ...
  var setFilter = function (path, filter, cb) {
    db.run(
      "INSERT OR REPLACE INTO filters (ID, path, filter) VALUES ((SELECT ID FROM filters WHERE path = ?), ?, ?)",
      path,
      path,
      filter,
      function (err) {
  ...
  var addAttachment = function (
    ...
    db.run(
      "INSERT INTO attachments (path, file_name, file_size, file_modifiedDate, saved_path) VALUES (?, ?, ?, ?, ?)",
      path,
      file_name,
      file_size,
      file_modifiedDate,
  ...
  var getAttachments = function (path, cb) {
    db.all(
      "SELECT * FROM attachments WHERE path=?",
      path,
      function (err, rows) {
        if (typeof cb != "undefined") {
          cb(err, rows);
