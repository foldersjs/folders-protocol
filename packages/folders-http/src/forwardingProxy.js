  ...
  if (conn.location.pathname === "/set_files") {
    helpers.objectifyPostData(conn.request.content).then(function (obj) {
      ...
      if (obj.shareId == "") {
        ...
        conn.request.setHeader(
          "content-length",
          parseInt(conn.request.getHeader("content-length")) + sizeIncrement,
        );
      }
    ...
    helpers.objectifyPostData(conn.request.content).then(function (obj) {
      if (obj.shareId == "") {
        var t = function (o, message) {
          if (o.shareId) {
            if (message.getHeader("set-cookie")) {
              self.sessions[o.shareId] = message
                .getHeader("set-cookie")[0]
                .toString()
                .split(";")[0];
              console.log(
...
ForwardingProxy.prototype.mode3Handler = function (conn, response) {
  self = this;
  var token;
  var options = helpers.requestObject(
  ...
  if (!self.sessions) {
    ...
    conn.request.content
      .pipe(outbound(options))
      .on("response", function (result) {
        token = stringifyCookie("FIOSESSIONID", {
          size: 32,
          Domain: ".folders.io",
          Path: "/",
        });
        if (result.headers["set-cookie"]) {
          ...
          self.sessions[token] = stringifyCookie("FIOSESSIONID", {
            value: conn.request.cookies["FIOSESSIONID"],
            Domain: ".folders.io",
            Path: "/",
          });
        }
...
ForwardingProxy.prototype.defaultModeHandler = function (conn, response, t) {
  ...
  conn.request.content
    .pipe(
      outbound(options).on("response", function (result) {
        if (t) {
